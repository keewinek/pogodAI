 ...
 * Usage: deno run -A automation/build-forecast.ts <locationId> <openMeteoJsonPath> [outputPath]
 */
...
function synthPrecip(probs: number[]): number {
...
function buildHourSlots(data: Record<string, unknown>): HourSlot[] {
  ...
  for (let i = 0; i < times.length; i++) {
    ...
    for (const m of MODELS) {
      const t = hourly[`temperature_2m_${m}`] as (number | null)[];
      const p = hourly[`precipitation_probability_${m}`] as (number | null)[];
      const w = hourly[`wind_speed_10m_${m}`] as (number | null)[];
      const c = hourly[`weather_code_${m}`] as (number | null)[];
...
function groupByDate(slots: HourSlot[]): Map<string, HourSlot[]> {
...
function pickHoursForDay(dayIndex: number, daySlots: HourSlot[]): HourSlot[] {
  if (dayIndex <= 2) {
    return daySlots.slice(0, 24);
  }
  const picked: HourSlot[] = [];
...
function buildDailySummary(daySlots: HourSlot[]) {
...
const locationId = Deno.args[0] ?? "szczecin";
const inputPath = Deno.args[1] ?? "/tmp/open-meteo-szczecin.json";
const outputPath = Deno.args[2] ?? "/tmp/forecast-szczecin.json";
...
const nowHour = allSlots.find((s) => s.time === (raw.current as { time: string }).time.slice(0, 16)) ??
...
await Deno.writeTextFile(outputPath, JSON.stringify(forecast));
console.log(`Written ${outputPath} (${JSON.stringify(forecast).length} bytes, ${days.length} days, ${sources.length} sources)`);
