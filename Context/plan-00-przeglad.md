...
> Dokument nadrzędny. Szczegóły w pozostałych plikach `plan-*.md` w tym folderze.

## 1. Cel

Osobisty, hiperlokalny system prognozowania pogody. Zamiast jednego, często niedokładnego źródła — agregacja danych z wielu serwisów pogodowych (TVN Meteo, Interia Pogoda, Google Weather, ICM/meteo.pl itp.) i synteza przez model AI w jeden, "ostateczny werdykt" dla wybranej lokalizacji.
...
1. **Agregator/Analityk** — Cursor Cloud Automation (cron). Szczegóły: `plan-05-automatyzacja-agent.md`.
2. **Backend/API** — endpointy Fresh w `routes/api/`. Szczegóły: `plan-04-api.md`.
3. **Frontend** — strona główna jak nowoczesna aplikacja pogodowa + edycja lokalizacji. Szczegóły: `plan-01-ui.md`, `plan-02-ux.md`.
4. **Infrastruktura/serwery** — Deno Deploy, KV, sekrety, domeny. Szczegóły: `plan-03-infrastruktura-serwery.md`.
