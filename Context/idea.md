...
pogody dla wybranej lokalizacji (np. Białołęka, Warszawa). Zamiast polegać na
jednym, często niedokładnym źródle, system agreguje dane z wielu serwisów
pogodowych (TVN, Interia, Google itp.) + wielu modeli matematycznych i przy
użyciu modelu AI wyciąga z nich jeden, "ostateczny" werdykt.
...
- Aby ominąć zabezpieczenia (Cloudflare, RODO), każda strona jest pobierana za
  pośrednictwem `https://r.jina.ai/` (konwersja do czystego Markdowna).
...
2. **Backend/Frontend (Deno Deploy, ten projekt Git):**

- Serwer zbudowany w TypeScript (Deno).
- Odbiera dane przez `POST`.
...
- **Bezpieczeństwo:** Aplikacja prywatna, nieindeksowana — bez autoryzacji API.
...
Dodatkowo, na stronie internetowej będzie możliwość wyboru aktualnej
lokalizacji. DOCELOWO będziemy chcieli mieć listę kilku lokalizacji, którą ja
będę mógł na stronie internetowej edytować (dodawać usuwać nowe lokalizacje do
zapisywania pogody w bazie danych)

Strona internetowa będzie działała jak zwykła aplikacja pogodowa, będzie
wyświetlać lokazlizację, którą wybierze użytkownik, lokalizacje będzie można
zmieniać w aplikacji, z pośród listy lokalizacji.
